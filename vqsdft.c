 ...
 * Embedded-friendly port of https://gist.github.com/TF3RDL/1810c570bad4f75f77941c981c65723f by TF3RDL
 * This algorithm is derived from the paper "Application of Improved Sliding DFT Algorithm for Non-Integer k" by Carl Q. Howard
...
void vqsdft_init(VQsDFT *v, const FreqBand* bands, int num_bands, const double* window, int window_len, double timeRes, int sampleRate) {
    ...
    for(int i = 0; i <= BUFFER_SIZE - 1; i++) v->buffer[i] = 0;
    for(int i = 0; i < MAX_BANDS; i++) v->spectrumData[i] = 0;

    for (int b = 0; b < num_bands; b++) {
        sDFT_Coeff *c = &v->coeffs[b];

        double periodFloat = sampleRate / (fabs(bands[b].hi - bands[b].lo) + 1.0 / (timeRes / 1000.0));
...
void vqsdft_analyze_block(VQsDFT *v, const int32_t *samples_q16, int num_samples) {
    for(int i = 0; i < v->num_coeffs; i++) v->spectrumData[i] = 0;
    ...
    for (int s = 0; s < num_samples; s++) {
        ...
        for (int i = 0; i < v->num_coeffs; i++) {
            ...
            if (oldestIdx < 0) oldestIdx += BUFFER_SIZE;
            ...
            for (int j = 0; j < coeff->kernel_length; j++) {
                ...
                int32_t c1x = MUL_Q(combX, coeff->twiddles[j].x) - MUL_Q(combY, coeff->twiddles[j].y) - coeff->coeffs2[j].x;
                int32_t c1y = MUL_Q(combX, coeff->twiddles[j].y) + MUL_Q(combY, coeff->twiddles[j].x) - coeff->coeffs2[j].y;
                ...
                int32_t c3x = c1x + MUL_Q(coeff->resonCoeffs[j], coeff->coeffs4[j].x) - coeff->coeffs5[j].x;
                int32_t c3y = c1y + MUL_Q(coeff->resonCoeffs[j], coeff->coeffs4[j].y) - coeff->coeffs5[j].y;
            ...
            if (v->spectrumData[i] < magSq) v->spectrumData[i] = magSq;
        }
    }
...
int main() {
    ...
    for (i = 0; i < dft_instance.num_coeffs; i++)
        printf("band %d\t(%.2f Hz):\t%f\n",
            i, bands[i].ctr, Q16_TO_FLOAT(dft_instance.spectrumData[i]));
